...
スマホ ⇄ Rokid RG-glasses を **Cloud XR Link (CXRL)** で繋ぐ双方向通信の最小サンプル。グラスのジェスチャ操作 (タップ / スワイプ) でメッセージ表示を制御し、スマホ側で操作ログをリアルタイム表示する。
...
| 役割 | 認証 / 接続維持 / ログ受信表示 | UI 描画 / ジェスチャ入力 / イベント送信 |
